...
var scheme = {
  ...
  buildMessageToSign: function(req) {
    ...
    var body = typeof req.original.body == 'string' ? req.original.body : JSON.stringify(req.original._body);
  ...
  sign: function(req, credentials) {
    ...
    var kDate = this._hmac(getCredentialDate(req.original.headers['date']), 'AWS4' + credentials.secret)
