...
function Hmmac(opts) {
  ...
  for (var i in DEFAULTS) {
    this.config[i] = opts.hasOwnProperty(i) ? opts[i] : DEFAULTS[i];
  }
...
Hmmac.prototype.validateSync = function(req, credentials) {
  req = this._wrap(req);
  if (!this.isRequestish(req) || !req.original.headers['authorization'] || !this.isCredentials(credentials)) return false;
...
Hmmac.prototype.validate = function(req, callback) {
  req = this._wrap(req);
  if (!this.isRequestish(req) || !req.original.headers['authorization'] || typeof callback != 'function') return callback(false);
...
Hmmac.prototype.sign = function(req, credentials) {
  req = this._wrap(req);
  ...
  req.original.headers[auth.key] = auth.value;
};
...
Hmmac.prototype._validate = function(req, credentials, parsedAuth) {
  ...
  var compareSignature = this._sign(req, credentials)
    , passedSignature = parsedAuth.signature;
...
Hmmac.prototype._sign = function(req, credentials) {
  ...
  try {
    ...
    this.log(err);
    throw new Error('Hmmac::_sign: Unable to sign the request');
  }
};
