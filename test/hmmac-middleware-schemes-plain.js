...
describe(`Middleware scheme(${scheme})`, () => {
  it('should succeed for authorized requests', () =>
    new Promise((done) => {
      ...
      middleware(mocks.plain.signedRequestFrozen, res, (err) => {
        assert.ifError(err);
  ...
  it('should fail with a 401 status code for unauthorized requests', () =>
    new Promise((done) => {
      ...
      middleware(mocks.plain.invalidSignedRequestFrozen, res, (err) => {
  ...
  it('should fail with a 401 status code for requests without any authorization', () =>
    new Promise((done) => {
      ...
      middleware(mocks.plain.unsignedRequestFrozen, res, (err) => {
        assert.strictEqual(true, err instanceof Error);
        assert.strictEqual(res.statusCode, 401);
        done();
      });
